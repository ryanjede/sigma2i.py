...
def test_ed_demo_runs():
    ...
    result = demo_tfim(n=TEST_NS[0], bc=TEST_BC, plot=PLOT)
    ...
    assert np.all(np.isfinite(result.sigma2)), "Variance must be finite"


def test_ed_peak_below_critical():
    """Placeholder: σ²_I peak occurs below h_c = 1.0 for small OBC chains."""
...
def test_variance_non_negative():
    ...
    for n in TEST_NS[1:]:
        result = demo_tfim(n=n, bc=TEST_BC, plot=PLOT)
        assert np.all(result.sigma2 >= 0), f"σ²_I negative somewhere in scan for n={n}: {result.sigma2}"
...
if __name__ == "__main__":
    ...
    sys.exit(1 if failed else 0)    for h in h_values:
